...
class MainApp(object):
    ...
    def changeUjiPresepsi(self, pos):
        try:
            ...
            jml = int(jml)
            if(jml > 0):
                query = """UPDATE tbl_uji_presepsi SET total_presepsi = {0} WHERE id_percobaan = {1}""".format(
                    jml, pos)
    ...
    def VPresepsi(self, data, nomor):
        ...
        btn_del = temp_view["btn_del"]
        btn_view = temp_view["btn_view"]

        btn_click = [btn_view, btn_del]
        id_click = data["id_percobaan"]
        for index in range(len(btn_click)):
            btn_click[index].disconnect()
            btn_click[index].clicked.connect(
                lambda _, x=[index, 1, id_click]: self.functionV_History(x))
    ...
    def VHistory(self, data, nomor):
        ...
        for index in range(len(btn_click)):
            btn_click[index].disconnect()
            btn_click[index].clicked.connect(
                lambda _, x=[index, 0, id_click]: self.functionV_History(x))
...
class Astar(QtCore.QThread):
    ...
    def run(self):
        ...
        if(fin <= 1.5):
            ...
            enorm_locking = 0.65  # 0.75  # 0.75  # 0.6375
...
if __name__ == "__main__":
    try:
        ...
        file = os.path.join(path, r'path_planning.db.bak_final')
